...
The `Fallback` command defines what the physical outlet (EB832/EB8) should do if it loses communication with the Brain (Apex Base Unit).
- **Safety Rule:** Life-support equipment (Return Pumps, Heaters) should typically be set to `Fallback ON`, while non-critical or hazardous-if-stuck-on equipment (Skimmers, ATOs, Dosers) should be set to `Fallback OFF`. [TIER-1]
...
| Command | Function | Default Usage | Source |
|---------|----------|---------------|--------|
| Set | Default State | `Set OFF` (Safety First) | [TIER-2] |
| If Temp > X | Safety Shutoff | Range: 27.5°C - 28.5°C | [TIER-1] |
| Fallback | Comm Loss | `ON` (Critical) / `OFF` (Hazard) | [TIER-1] |
| Defer | Noise Filter | `001:00` (Typical ATO) | [TIER-2] |
| Min Time | Cycle Protect | `005:00` (Typical Heater) | [TIER-2] |
...
- **Drip Loops:** While not software, all Apex modules must be installed with physical drip loops to prevent water intrusion into the AquaBus/Power ports. [TIER-1]
...
- [CONFLICT] Manufacturers recommend `Fallback OFF` for return pumps in some manuals, but Triton methodology prioritizes oxygenation/flow (`Fallback ON`). [TIER-1/2 Conflict]
