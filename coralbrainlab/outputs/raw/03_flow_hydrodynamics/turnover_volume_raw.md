...
**Volume** serves as the system's thermal and chemical "buffer." A larger volume of water is inherently more stable because changes in temperature or chemical concentration are diluted, providing a slower rate of change that allows delicate reef inhabitants to habituate. **Turnover** refers to the speed at which this volume is processed through filtration (sump turnover) and circulated within the display (internal flow). High turnover ensures that there are no "stagnant zones" where oxygen can deplete or waste can accumulate, mimicking the high-energy environments where most reef-building corals thrive.
...
*   **Triton Recommendation:** 10x total system volume per hour (after head loss) [TIER-1: Triton official documentation].
*   **Rationale:** This high flow rate ensures that the macroalgae in the refugium have constant access to nutrients and that the entire water volume is frequently exposed to the biological and mechanical filtration processes in the sump.
