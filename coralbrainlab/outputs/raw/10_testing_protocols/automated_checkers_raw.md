...
The "why" of automation is rooted in the **Stability over Numbers** principle. Corals are highly adaptable to specific parameter setpoints, but they are extremely sensitive to *rate of change*. A sudden 1.0 dKH swing caused by a human error in manual testing or a failed dosing pump can trigger Tissue Necrosis (RTN/STN). Automated checkers act as a persistent biological heartbeat monitor, allowing for micro-adjustments in dosing that mimic the steady-state chemistry of the open ocean.
...
*   **Discrete (Titration):** Devices like the Trident or Alkatronic pull a discrete sample, test it, and then clear the lines. This is highly accurate for Alkalinity, Calcium, and Magnesium.
*   **Continuous (ISE Probes):** Devices like the Ion Director use Ion-Selective Electrodes (ISE) to potentially monitor parameters continuously. While promising for Nitrate and Calcium, these sensors require frequent calibration and are sensitive to electrical interference and "drift."
[TIER-2: BRS Investigates - Automated Testing Comparison]

### The "Control Loop" (Closing the Circle)
The true power of an automated checker is not just measurement, but **Control**. When integrated with an aquarium controller (Apex, ProfiLux, or standalone logic), the checker can dynamically adjust dosing pump speeds. If alkalinity is detected to be falling, the controller can slightly increase the dose of TRITON Core7 or Balling salts to compensate in real-time.
...
3.  **Warning:** One should never allow an automated system to adjust dosing by more than 10-15% without a manual safety check, to prevent "runaway" dosing caused by a clogged intake or faulty reagent.
[TIER-2: Triton Method Guidelines for Automation]

### Common Devices & Comparison

| Device | Primary Parameters | Integration | Reagent Type |
|--------|-------------------|-------------|--------------|
| **Neptune Trident** | Alk, Ca, Mg | Apex Only | Proprietary (Monthly) |
| **Alkatronic** | Alk Only | Standalone/Apex/GHL | Open/DIY Reagent |
| **GHL KH Director** | Alk Only | GHL/ProfiLux | Lab Grade Reagent |
| **Mastertronic** | Alk, Ca, Mg, NO3, PO4... | Standalone | Test Kit Based |
| **Ion Director** | Ca, Mg, NO3, K, Na | GHL | ISE Calibration Fluid |

### Maintenance & Calibration
Automation does not mean "set and forget."
*   **Reagent Freshness:** Expired or settled reagents will produce false trends.
*   **Probe Calibration:** pH probes inside checkers must be calibrated monthly to ensure titration endpoints remain accurate.
*   **Line Cleaning:** Intake lines can develop biofilm or salt creep, significantly impacting the precision of the sample volume.
[TIER-2: Manufacturer Maintenance Schedules]
...
| Nitrate/Phos| 1 - 2 tests / day | Adjust Carbon Dose / Export | [VERIFY] |

## Safety Considerations

⚠️ **TIER 1 CONSTRAINTS (Absolute):**
- **Maximum Automated Correction:** Never allow an automated system to adjust dosing by more than **1.5 dKH total per day**, even if the probe suggests a larger correction is needed. This prevents "probe death" from killing the tank.
- **Verification Rule:** If an automated reading shows a deviation >0.5 dKH from the previous reading, the system must **re-test immediately** and/or alert the user for a manual verification before taking corrective action.

⚠️ **TIER 2 DEFAULTS:**
- **Waste Management:** Ensure reagent waste is collected in a separate container and never returned to the tank (unless using a device specifically designed for it, like Alkatronic in certain modes). Reagent toxicity varies.
- **Fail-Safe:** Always set "high/low" shutoff limits. If Alk > 9.0 or < 6.5, the doser should be disabled and an emergency alarm triggered.
...
- [GAP] Compatibility of Mastertronic with non-standard test kits.
