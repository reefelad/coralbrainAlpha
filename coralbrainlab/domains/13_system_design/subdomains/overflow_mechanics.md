...
## Overview
The overflow and drain system is a critical hydrostatic interface between the display tank and the filtration sump. Biologically, its primary function is **surface renewal**. Organic pollutants, proteins, and lipids are naturally buoyant and accumulate at the air-water interface, forming a surface film. If left undisturbed, this film inhibits gas exchange (O2 in, CO2 out) and reduces light penetration. By skimming the surface water into an overflow box, the system ensures that these organics are continuously transported to the protein skimmer for export, maintaining a clean and oxygen-rich environment for the reef inhabitants.
...
1. **Gate Valve Requirement:** Always use a high-quality gate valve for the main siphon. **Ball valves are unsuitable** for this application because they lack the fine resolution required to balance the siphon against the return flow; a tiny movement in a ball valve creates a massive change in flow, leading to constant "chasing" of the water level. [TIER-2: Consensus]
2. **Submerged Exit:** The exit of the main siphon in the sump must be submerged (typically 1-2 inches) to prevent air from traveling back up and breaking the siphon.
...
- **Maintenance Checks:** Visually inspect overflow boxes and strainers weekly for salt creep, algae growth, or calcification that could restrict flow. [TIER-2]
...
- Previous: [None]
- Next: [sump_design](sump_design.md)
