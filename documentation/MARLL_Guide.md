...
*"Pop quiz, hotshot: you've got 166 subdomains and 4 agents. What do you do?"*
...
YOUR ONE JOB:
1. Check orchestration/state.json for the current subdomain
2. Do a web search for reliable information
3. Write the raw content following the template in your skill
4. Save to: outputs/raw/[domain]/[subdomain]_raw.md
5. Move to the next subdomain immediately

KEEP GOING until you run out of subdomains or I tell you to stop.
...
YOUR ONE JOB:
1. Check outputs/raw/ for any files ending in _raw.md
2. If there's a matching _critique.md in outputs/critiqued/ → skip it
3. If there's NO matching critique → read the raw file and critique it
4. Save your critique to: outputs/critiqued/[domain]/[subdomain]_critique.md

KEEP CHECKING every few minutes for new raw files to critique.
...
YOUR ONE JOB:
1. Check outputs/critiqued/ for any files ending in _critique.md
2. If there's a matching _refined.md in outputs/refined/ → skip it
3. If there's NO matching refined file → read BOTH the raw AND critique, then refine
4. Save to: outputs/refined/[domain]/[subdomain]_refined.md

KEEP CHECKING every few minutes for new critiques to refine.
...
YOUR ONE JOB:
1. Check outputs/refined/ for any files ending in _refined.md
2. If there's a matching file in outputs/final/ → skip it
3. If there's NO final version → format it and save to BOTH:
   - domains/[domain]/subdomains/[subdomain].md
   - outputs/final/[domain]/[subdomain].md
4. Update orchestration/state.json to mark it completed

KEEP CHECKING every few minutes for new refined files to compile.
